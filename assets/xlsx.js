  ...
  Xlsx.exportModel = function (model, inputs, opts) {
    ...
    const info = [
      ...
      .concat([['', ''], ['导出说明',
        ...
        /* 以前一律写「所有历史财务数据来自年报 / 10-K」，构造数据的模型导出来也这么说 */
        (global.Prov && global.Prov.isSynthetic && global.Prov.isSynthetic(model)
          ? '本模型的数据为教学构造，不对应任何真实公司或交易。'
          : '历史数据的出处与核对情况见本页「数据说明」，预测与假设已在表内标注。') +
        '仅供学习使用，不构成投资建议。']]);
